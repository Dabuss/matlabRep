function [x,fval,exitflag,output,lambda,grad,hessian] = IKThroughOptim11SimpleButEfficientWithBounds(vrep_store)
...
[x,fval,exitflag,output,lambda,grad,hessian] = fmincon(@(x)0,x0,[],[],[],[],lb,ub,@(x)IKConstraints(x,vrep_store,0,0),options);
